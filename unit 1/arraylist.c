...
int main()
{
 ...
 {
   int i;
   //shift all the elements to right to make space for the element at location 0
...
void delete_element(struct LIST *l, int x)
{
 ...
 for(i=0;i<l->count;i++)
 {
   if(l->first[i]==x)
   {
    ...
    for(k=i+1;k<l->count;k++) //shift all elements after the deleted element towards left
